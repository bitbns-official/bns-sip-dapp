...
contract SIP {
    ...
    function chargeSppIndex(address pair, uint256 start, uint256 end, bool _upwards) external _ownerOnly _ifNotLocked discountCHI {
        ...
        if(sppList.length <= end){
            end = sppList.length - 1;
            _pairData.position = 0;
        }
        else {
            _pairData.position = end;
        }
        
        chargeSppStruct[] memory sppCharged = new chargeSppStruct[]((end - start + 1));
        
        uint successIndex = 0;
        
        for(uint256 i=start; i<=end; i++){

            if(inactiveSIP[sppList[i]]){
                continue; // SIP not active - No need to load data
            }
            sppSubscribers storage _subscriptionData = sppSubscriptionStats[sppList[i]];
    ...
    function chargeSppIndexWithSuccessCount(address pair, uint256 start, uint256 toCharge, uint256[] memory _exceptions, bool _upwards) external _ownerOnly _ifNotLocked discountCHI {
        ...
        require(sppList.length > start && sppList.length!=0, "No SIP to charge");
        ...
        if(toCharge > (sppList.length - start)){
            toCharge = (sppList.length - start) + 1;
        }
        
        chargeSppStruct[] memory sppCharged = new chargeSppStruct[]((toCharge + 1));
        
        uint successIndex = 0;
        uint8 doneTill = 0;
        
        for(uint256 i=start; i<=(sppList.length-1); i++){

            if(_exceptions.length > doneTill && _exceptions[doneTill]==sppList[i]){
                doneTill++;
                continue; // Marked as exception in inputs - dont load
            }
            // if(inactiveSIP[sppList[i]]){
            //     continue; // SIP not active - No need to load data
            // }
            ...
            if(successIndex >= toCharge){
                break; // List is ready, lets charge it now
            }
        }
    ...
    function chargeSppAll(address pair, uint256 toCharge, bool _upwards) external _ownerOnly _ifNotLocked discountCHI {

        uint256 gasStart = 21000 + gasleft() + 3000 +  (16 * msg.data.length);
        
        uint256[] memory result;
        pairStats storage _pairData = map1[pair];  // 800
        
        if(!_upwards){
           _pairData = map2[pair]; 
        }
        
        uint256[] storage sppList = _pairData.sppList; // 800
        ...
        uint256 end = (_pairData.position).add(toCharge);
        ...
        chargeSppStruct[] memory sppCharged = new chargeSppStruct[]((end - (_pairData.position) + 1));
        
        
        
        for(uint256 i=(_pairData.position); i<=end; i++){
        ...
        uint256 txFee = (gasStart - gasleft() +  (successIndex * 50000)) * tx.gasprice;
        uint256 _feeDed = txFee; // reusing end variable here, making a new variable giives stack too deep
